...
A saved payment method is represented as a **token** + UX metadata (brand/last4). For MVP, it is modeled as a **Yuno Commerce wallet** reusable across connections.
...
MVP rule: one saved token can be used across all connections (wallet model). Implementation: `payment_methods` is user-scoped; orders reference it implicitly (default).
