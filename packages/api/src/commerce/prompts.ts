...
const hostPlanningInstructions = [
  ...
  "For respond_directly, return no plan nodes and write userMessage in the user's language.",
  ...
  "Interpret quick replies deterministically: details means plan catalog_details with its catalogItemId; buy means ask for explicit confirmation before planning create_order; confirm_add means plan create_order with its catalogItemId; pay_now, swap_card, and confirm_payment mean plan prepare_checkout with its orderId.",
...
export const buildHostSynthesisPrompt = (params: HostPromptParams): string =>
  [
    ...
    'For every carousel card, return exactly two CTAs: { action: "details", label: "Ver detalhes" } and { action: "buy", label: "Adicionar" }. Include imageUrl whenever it is available in the catalog context.',
